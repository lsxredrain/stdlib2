...
pick(Xs) -> lists:nth(rand:uniform(length(Xs) + 1), Xs).
...
shuffle(Xs) ->
  [X || {_N, X} <- lists:keysort(1, [{rand:uniform(length(Xs)), X} ||
                                      X <- Xs])].
